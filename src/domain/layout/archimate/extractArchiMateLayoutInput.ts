...
function sizeForLayoutNode(n: ViewNodeLayout): { width: number; height: number } {
  ...
  const d = isConnector ? DEFAULT_CONNECTOR_SIZE : DEFAULT_ELEMENT_SIZE;
...
function edgesFromLegacyLayout(view: View, model: Model, nodeIdSet: Set<string>): LayoutEdgeInput[] {
...
/**
 * Extract a layout graph (nodes + edges) from a specific ArchiMate view.
 *
 * This function is intentionally pure:
 * - It reads from the model/view
 * - It does not mutate anything
 * - It does not depend on React/store state
 */
export function extractArchiMateLayoutInput(
  model: Model,
  viewId: string,
  ...
  if (!view) throw new Error(`extractArchiMateLayoutInput: view not found: ${viewId}`);
