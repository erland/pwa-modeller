...
function tryRenderSandboxSvgAsShapes(
  ...
  try {
    ...
    const roundRect = ShapeType.roundRect;
    ...
    if (edges.length > 0) {
      ...
      for (const e of edges) {
        ...
        const forcedHeadEnd =
          relLower.includes('composition') || relLower.includes('aggregation') ? 'diamond' : (e.pptxHeadEnd ?? undefined);
        const forcedTailEnd =
          relLower.includes('composition') || relLower.includes('aggregation') ? 'none' : (e.pptxTailEnd ?? undefined);
