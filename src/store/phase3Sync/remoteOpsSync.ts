...
import { applyEventsSequentially, ensureModel, extractModelFromSnapshotPayload, setInFlightOpIds } from './applyRemoteOps';
import type {
...
export function createRemoteOpsSyncController(deps: RemoteOpsSyncDeps): RemoteOpsSyncController {
  ...
  async function runLoop(localDatasetId: DatasetId, serverDatasetId: string): Promise<void> {
    ...
    while (true) {
      ...
      try {
        const { fromRevision: from } = computeSyncPlan({ lastAppliedRevision: getLastAppliedRevision(localDatasetId) });
...
export { extractModelFromSnapshotPayload } from './applyRemoteOps';
